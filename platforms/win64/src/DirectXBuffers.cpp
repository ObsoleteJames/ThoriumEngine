...
DirectXBuffer::DirectXBuffer(const FBufferDescriptor& desc)
{
	this->desc = desc;
	...
	buffDesc.StructureByteStride = desc.dataStride;
	buffDesc.BindFlags = bindFlags[desc.type];
	
	if (desc.type == TH_BUFFER_TYPE_SHADER_BUFFER)
		buffDesc.ByteWidth = (uint)(desc.bufferSize % 16 == 0 ? desc.bufferSize : desc.bufferSize + (16 - (desc.bufferSize % 16)));

	D3D11_SUBRESOURCE_DATA data{};
	data.pSysMem = desc.data;

	IRenderer::LockGPU();
	HRESULT hr = GetDirectXRenderer()->device->CreateBuffer(&buffDesc, desc.data ? &data : nullptr, &buffer);
	THORIUM_ASSERT(SUCCEEDED(hr), "Failed to create DirectX vertex buffer");
	IRenderer::UnlockGPU();
}
...
void DirectXBuffer::Update(SizeType ammount, void* data, SizeType offset)
{
	D3D11_MAPPED_SUBRESOURCE target;
	GetDirectXRenderer()->deviceContext->Map(buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &target);
	memcpy((void*)((SizeType)target.pData + offset), data, ammount);
	GetDirectXRenderer()->deviceContext->Unmap(buffer, 0);
}
