...
DirectXVertexShader::DirectXVertexShader(CShaderSource* in, FString path)
{
	...
	if (FAILED(hr))
		CONSOLE_LogError("IShader", "Failed to create input layout for vertex shader '" + in->Name() + "'");

	hr = GetDirectXRenderer()->device->CreateVertexShader(buff->GetBufferPointer(), buff->GetBufferSize(), nullptr, &shader);
	if (FAILED(hr))
	{
		CONSOLE_LogError("IShader", "Failed to create vertex shader '" + in->Name() + "'");
		buff->Release();
		return;
	}
...
DirectXSkinnedVertexShader::DirectXSkinnedVertexShader(CShaderSource* in, FString path)
{
	bValid = false;
	shaderSource = in;
	type = 0;
	...
	D3D11_INPUT_ELEMENT_DESC vertexLayout[] = {
		{ "POSITION",		0, DXGI_FORMAT_R32G32B32_FLOAT,		0, offsetof(FSkinnedVertex, position),		D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{ "NORMAL",			0, DXGI_FORMAT_R32G32B32_FLOAT,		0, offsetof(FSkinnedVertex, normal),		D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{ "TANGENT",		0, DXGI_FORMAT_R32G32B32_FLOAT,		0, offsetof(FSkinnedVertex, tangent),		D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{ "COLOR",			0, DXGI_FORMAT_R32G32B32_FLOAT,		0, offsetof(FSkinnedVertex, color),			D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{ "TEXCOORD",		0, DXGI_FORMAT_R32G32_FLOAT,		0, offsetof(FSkinnedVertex, uv1),			D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{ "TEXCOORD",		1, DXGI_FORMAT_R32G32_FLOAT,		0, offsetof(FSkinnedVertex, uv2),			D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{ "BONEINDICES",	0, DXGI_FORMAT_R32G32B32A32_SINT,	0, offsetof(FSkinnedVertex, bones),			D3D11_INPUT_PER_VERTEX_DATA, 0  },
		{ "BONEWEIGHT",		0, DXGI_FORMAT_R32G32B32A32_FLOAT,	0, offsetof(FSkinnedVertex, boneInfluence),	D3D11_INPUT_PER_VERTEX_DATA, 0  },
	};
	constexpr UINT vertexLayoutSize = ARRAYSIZE(vertexLayout);
...
DirectXSkinnedVertexShader::~DirectXSkinnedVertexShader()
{
	if (shader)
		shader->Release();
	if (inputLayout)
