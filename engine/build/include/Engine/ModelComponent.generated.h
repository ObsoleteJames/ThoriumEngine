
#include "Object/Class.h"
#include "Object/ObjectMacros.h"

#define Engine_ModelComponent_h_24_GeneratedBody \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, model) \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, materials) \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, bCastShadows) \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, animationType) \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, animationAsset) \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, animationGraph) \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, activeBodyGroups) \
PRIVATE_MEMBER_OFFSET_ACCESSOR(CModelComponent, skeleton) \
DECLARE_EXEC_FUNCTION(OnModelEdit)\
DECLARE_CLASS(CModelComponent, CPrimitiveComponent, Engine)


#undef FILE_ID
#define FILE_ID Engine_ModelComponent_h
