...
class CTextureStreamingProxy : public IAssetStreamingProxy
{
public:
	...
	void Load() override
	{
		...
		SizeType offset = sizeof(FAssetHeader) + sizeof(ETextureAssetFormat) + 4 + 4 + 1;
...
void CTexture::OnInit(IBaseFStream* stream)
{
...
void CTexture::OnLoad(IBaseFStream* stream, uint8 lodLevel)
{
	if (numMipmaps > 1)
	{
		...
		CAssetManager::StreamAsset(new CTextureStreamingProxy(this, lodLevel));
		return;
	}

	SizeType offset = sizeof(format) + 4 + 4 + 1;
	if (version != THTEX_VERSION_01)
