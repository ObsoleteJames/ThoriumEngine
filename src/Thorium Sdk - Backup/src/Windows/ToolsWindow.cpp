...
FString& CToolsWindow::GetStyleSheet()
{
	...
	{
		FString stylePath = ".\\bin\\styles\\style.h";
		FString enginePath = ".";

		std::ifstream stream(stylePath.c_str());
		if (!stream.is_open())
		{
			enginePath = ToFString(CEngine::OSGetEnginePath(ENGINE_VERSION));
			stylePath = enginePath + "\\bin\\styles\\style.h";
			stream.open(stylePath.c_str());
		}

		THORIUM_ASSERT(stream.is_open(), "Failed to open 'style.h'");
