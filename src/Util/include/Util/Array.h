...
public:
	...
	const TReverseIterator<T> operator++() const { return TReverseIterator<T>(ptr - 1); }
	const TReverseIterator<T> operator++(int) const { return TReverseIterator<T>(ptr - 1); }
...
public:
	...
	//const TIterator<T>& operator--() { ptr--; return *this; }
	//const TIterator<T> operator--(int) { ptr--; return *this; }
...
public:
	...
	SizeType Index(const TIterator<T>& it);
	SizeType Index(const TReverseIterator<T>& it);
