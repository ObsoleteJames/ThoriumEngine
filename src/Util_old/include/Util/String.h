...
public:
	...
	TArray<TBaseString<T>> Split(T ch, SizeType offset = 0);
	TArray<TBaseString<T>> Split(const TBaseString<T>& str, SizeType offset = 0);
...
template<typename T>
TArray<TBaseString<T>> TBaseString<T>::Split(T ch, SizeType offset /*= 0*/)
{
...
template<typename T>
TArray<TBaseString<T>> TBaseString<T>::Split(const TBaseString<T>& str, SizeType offset /*= 0*/)
{
	TArray<TBaseString<T>> arr;
	arr.Resize(1);
	for (auto& _c : *this)
...
namespace std
{
	...
	struct hash<TBaseString<wchar_t>>
	{
		...
		{
			return hash<const wchar_t*>()(str.c_str());
		}
	};
}
